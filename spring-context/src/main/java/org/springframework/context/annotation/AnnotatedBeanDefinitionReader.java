...
public class AnnotatedBeanDefinitionReader {
	...
	private <T> void doRegisterBean(Class<T> annotatedClass, @Nullable String name,
		...
		/**
		 * 根据传入的类创建一个spring内部描述类的对象AnnotatedGenericBeanDefinition
		 * 里面包含传入的类的一些信息，例如：scope、lazy等
		 */
		AnnotatedGenericBeanDefinition abd = new AnnotatedGenericBeanDefinition(annotatedClass);
		/**
		 * 判断这个类是否需要跳过解析
		 * 主要是判断该类有没有加Conditional注解
		 */
		...
		/**
		 * 判断是否有beanName，如果不存在，则去注解中拿取value值，拿不到则获取类名，首字母小写作为beanName
		 * 可以自己定义一个beanName的生成器，继承BeanNameGenerator即可
		*/
		String beanName = (name != null ? name : this.beanNameGenerator.generateBeanName(abd, this.registry));

		/**
		 * 加载一些类的通用注解，主要处理Lazy、DependsOn、Primary、Role等
		 * 处理完之后依然将它们添加到AnnotatedGenericBeanDefinition中
		 */
		AnnotationConfigUtils.processCommonDefinitionAnnotations(abd);

		/**
		 * 如果在向容器注册注解bean时，使用了额外的限定符注解则解析
		 * byName和qualifiers是Annotation型的数组，里面不仅仅是存Qualifier注解，
		 * 理论上里面存的是一切注解，所以需要循环判断里面是否包含了Primary和Lazy注解
		 */
		if (qualifiers != null) {
		...
		/**
		 * 生成一个新的数据结构BeanDefinitionHolder
		 */
		...
		/**
		 * 注册bean,把definitionHolder里包含的类信息注册到DefaultListableBeanFactory中
		 */
